...
p_co <- p_co + scale_x_continuous(limits = c(0,410)) + scale_y_continuous(limits = c(-0.11,0.8))
p_co <- p_co + geom_rect(data = data_co, aes(xmin = start, xmax = end, ymin = -0.11, ymax = 0, fill = Region),color = "transparent") +
  scale_fill_manual(values = c("PBD" = "#8ECFC9", "PD" = "#FFBE7A"," " = "transparent", "C2D" = "#FA7F6F", "CTT" = "#82B0D2")) +
  geom_text(aes(x = start + (end - start)/2, y = -0.09, label = Region), vjust = 0, color = 'black',size = 5) +
  geom_segment(aes(x = 185, y = -0.055, xend = 190, yend = -0.055), color = "grey", linetype = "solid", size = 1)+
  theme_minimal()
